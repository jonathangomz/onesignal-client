...
const { validate, validateString, configSchema, messageSchema, optViewNotificationsSchema, targetsSchema, optViewDevicesSchema } = require('./schemes');
...
class OneSignal {
   ...
   * 'segments' to select by segments. 'users' to select by OneSignal users ids. 'externals' to select by your own users ids.
   ...
   * @throws Will throw an error if some extra options are not valid for the call according to OneSignal documentation.
   */
   ...
   * @throws Will throw an error if the notification is already delivered.
   */
 ...
 /**
   ...
   * _For more information go to https://documentation.onesignal.com/reference/view-devices_.
   * @param  {Object} [options] - An object with the parameters options.
   * @param  {number} [options.limit] - How many devices to return.
   * Max is 300. Default is 100.
   * @param  {string} [options.offset] - Result offset.
   * Default is 0. Results are sorted by id.
   * @returns {Promise<Response>} - A promise with the response for the call. _See properties on https://visionmedia.github.io/superagent/#response-properties_.
   * @throws Will throw an error if limit is not a number.
   */
  async viewDevices(options) {
    validate(options, optViewDevicesSchema);
 ...
 /**
   ...
   * _For more information go to https://documentation.onesignal.com/reference/view-device_.
   * @param  {string} player_id - Player's OneSignal ID.
   * @returns {Promise<Response>} - A promise with the response for the call. _See properties on https://visionmedia.github.io/superagent/#response-properties_.
   * @throws Will throw an error if the 'player_id' parameter is not presented or is not a string.
   */
  async viewDevice(player_id) {
    validateString(player_id, 'player_id');

    const pathParams = util.generatePathParams({app_id: this.appId});

    const path = `${API_URL}/players/${player_id}`;
    return await Request
      .get(`${path}?${pathParams}`)
      .set('Authorization', `Basic ${this.restApiKey}`)
      .send();
